---
title: "How to reduce SaaS churn | Paddle"
description: "Learn about the top 5 strategies for SaaS revenue leaders to reduce customer churn rates (quantifiably!) in just 45 days."
source: "blog/reduce-churn.html"
---

# How to reduce SaaS churn | Paddle

![](https://images.prismic.io/paddle/e333ac6f-de8d-4463-9db2-1cde43626bdc_Yellow+on+Black+Mono.png?auto=compress%2Cformat&fit=max&w=3840)

Author

[Paddle](/resources/author/paddle)

Share

[](https://x.com/intent/tweet?text=How%20to%20reduce%20SaaS%20churn%3A%205%20strategies%20quantified%20in%20%24s%20and%20%25s%20-%20https%3A%2F%2Fwww.paddle.com%2Fblog%2Freduce-churn%20via%20%40paddlehq)[](https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fwww.paddle.com%2Fblog%2Freduce-churn)[](https://www.linkedin.com/shareArticle?mini=true&url=https%3A%2F%2Fwww.paddle.com%2Fblog%2Freduce-churn&title=How%20to%20reduce%20SaaS%20churn%3A%205%20strategies%20quantified%20in%20%24s%20and%20%25s&source=)
...
Just moving from 3% monthly churn to 2% monthly churn (calculated as  _(1-0.02)^12 - (1-0.03)^12)_ can translate into 9% less ARR churn in 12 months — or in other words, give you another month’s worth of revenue.

Here we intend to equip you with a clear, easy-to-prioritize set of options for [tackling customer churn](https://www.paddle.com/resources/state-of-saas-meta-report) that can and should deliver a quantifiable lift for your team within just 45 days.
...
In this guide:
  ...
  * [The 4 parts to every churn reduction strategy ](#4-part-churn-strategy)
  * [Should you optimize leading or lagging indicators of churn? ](#leading-vs-lagging-indicators)
  * [Strategy #1: Cancellation surveys & offers (5% less ARR churn) ](#strategy-1)
  * [Strategy #2: Dunning campaigns (10% less ARR churn) ](#strategy-2)
  * [Strategy #3: User activation campaigns (15% less ARR churn) ](#strategy-3)
  * [Strategy #4: “Upgrade to Annual” campaign (25% less ARR churn) ](#strategy-4)
  * [Strategy #5: Improve subscription payment acceptance (30% less ARR churn) ](#strategy-5)
  * [Prioritizing the right strategy for you ](#prioritizing-the-right-strategy)
  * [Putting it all together ](#putting-it-all-together)
  * [Churn reduction strategies FAQ ](#churn-reduction-strategies-FAQs)
...
You need to classify your churn strategies by:
  ...
  * **Involuntary, passive “delinquent” churn**(customer’s subscriptions canceling due to [failed payments](https://www.paddle.com/blog/6-reasons-for-low-payment-acceptance-in-saas))
...
You also need to classify your churn strategies by:
  ...
  * **Pre-churn event**(before the [subscription renewal](https://www.paddle.com/resources/subscription-renewal) is due)
...
With the four quadrants, you can broadly group your [churn reduction](https://www.paddle.com/resources/user-churn) strategies together (and spot where you’re missing out).

  1. **Keep users happy & active - **Nurture users to new feature adoption and annual plans.
  2. **Cancellation offers -** When customers click to cancel, find out why and make offers to deflect the cancellation request.
  3. **Payment acceptance -** Stop subscription payments from failing (and churn from happening accidentally).
  4. **Payment recovery** Recover failed credit card payments and fraudulent chargebacks


Your goal should be to build a complete churn reduction strategy across all four quadrants. That way, you’ll maximize your chances of [reducing churn](https://www.paddle.com/alternatives/subscription-billing-software) and improve revenue retention.

**What is a leading indicator?**

A metric or input that signals the future achievement of an objective. E.g. an increase in active users signals lower churn. 

**What is a lagging indicator?**

A metric or output that measures current performance. E.g. the number of cancellations.

## **Should you optimize leading indicators or lagging indicators of churn?**

The matrix helps you prioritize the fastest strategies you can prioritize to [reduce churn](https://www.paddle.com/blog/talking-churn) quickly.

You can split it into leading and lagging indicators.

The top-left quadrant is most likely to impact new customer cohorts and make an impact over the long run. For instance, happy and [active users](https://www.paddle.com/resources/daily-active-users) are a **leading indicator** of lower churn. It’s important to optimize but it’s harder to quantify their direct impact on revenue — particularly in a short time frame.
...
With that in mind, here are the **top five[churn reduction strategies](https://www.paddle.com/resources/reduce-customer-churn). **These estimates are based on evidence from benchmark data at Paddle and other providers. Your mileage may (will) vary, but this ought to give you some benchmark guidance to prioritize with.
...
In the context of why a loyal customer is cancelling, it can make sense to make a specific offer or discount. The most common incentive we see is to [pause the subscription](https://developer.paddle.com/guides/how-tos/subscriptions/cancel-and-pause) (saving the customer account, payment method, and contact details for possibly reactivating in the future) for 30 days or indefinitely.
...
Some Paddle sellers use [our subscription support team ](https://www.paddle.com/platform/managed-support)to proactively offer discounts to stay when cancellation requests come in. For larger customer accounts, offering 50% discounts if they move to an annual plan has resulted in a 2% “deflection” rate on cancellations.

[Paddle Retain](https://www.paddle.com/retain)'s Cancellation Flows help you save more customers by providing a customized and curated offboarding and salvage user experience.
...
Estimate annual impact = 5.3% decrease in year-end ARR churn
 ...
 _Compound the monthly impact as 1 - (1-0.0045)^12_

With [churn prevention](/resources/churn-prevention) offers, it’s important to calculate this in the context of revenue retention, not customer retention. You need to **factor in the costs of any discounts** in terms of revenue churn (e.g., 20% discount = 20% revenue churn if accepted).
...
In contrast, digital wallets like **PayPal can have multiple payment methods and act as a source of funds themselves**(i.e., your PayPal balance). Similarly, bank wire transfers are directly linked to the source of funds and would only decline if the account being debited was itself delinquent. Both of these lead to higher authorization rates and [payment acceptance](https://www.paddle.com/features/payment-acceptance).

_“To dun”_ literally means “ _to make persistent demand_ s.” Dunning programs are designed to recapture failed subscription payments. Generally, these split into two sides to help “resurrect” a customer account and [reduce churn](https://www.paddle.com/resources/saas-retention-guide):

  1. Payment automation (automatically retrying payments)
...
Some dunning providers will touch on multiple parts of this process like Churnbuster, Baremetrics Recover, and Paddle Retain suggest figures around different steps in their processes they touch. At Paddle, we see similar numbers for leftover failed payments with our own [dunning programs](https://www.paddle.com/features/dunning).
...
_Compound the monthly impact as 1 - (1-0.01)^12_
...
Your dunning email campaigns (if you have any) are likely to be tied to your subscription billing stack. You should review these with your marketing team to try to send more emails over a longer period of time, and review the content within each message.
  ...
  * Optimize the dunning emails within the [subscription billing tool](https://www.paddle.com/features/subscription-management) you already have.
  ...
  * Review dunning tools like [Paddle Retain](www.paddle.com/retain) or Churnbuster which are generally free to get started, but charge later based on revenue recovered.
...
Given a longer-term horizon, improving your user onboarding process can have a dramatic impact on customer retention and churn performance for your future customer cohorts. Your users need to hit their  _“aha!”_ moment and realize the value of your product for yourself.
...
Case studies on Appcues report [doubling their user engagement rate](https://www.appcues.com/blog/sitemate-engagement-rate-case-study?utm_source=paddle&utm_medium=referral&utm_campaign=q2-churn-reduction) among current customers with in-app walkthroughs and tooltips. 

### **Calculating the value to your business**

Leading indicators are very tricky to generalize, particularly in the context of near-term results. But we take the case study results from doubling [user engagement](/resources/customer-engagement) and carry that impact forward to reduce voluntary churn.
...
Estimate annual impact = 16.6% decrease in year-end ARR churn
 ...
 _Compound the monthly impact as 1 - (1-0.015)^12_
...
## **Strategy #4:_Upgrade to annual_ campaign (25% less ARR churn)**
...
Unbounce shared a customer marketing campaign where they moved [from 2% of customers on annual plans to 21% of customers ](https://www.slideshare.net/DistilledSEO/searchlove-san-diego-2018-alexa-hubley-real-lessons-in-growth-marketing-from-watching-romantic-comedies)over a three-month campaign.

Paddle reports a [negative correlation between the percentage of revenue on annual contracts and monthly churn](https://www.paddle.com/blog/annual-vs-monthly-billing). It’s not realistic to move every customer to an annual plan overnight, but it is likely that a concentrated campaign can move from one “bracket” to the next. Still, this can show a 2.4% decrease in monthly voluntary churn.
...
Estimate annual impact = 26.2% decrease in year-end ARR churn
 ...
 _Compound the monthly impact as 1 - (1-0.025)^12_
...
Another option could be card payments which offers a more streamline billing process, a more predictable billing cycle and reduces churn. Add a [tool for dunning and churn management](https://www.paddle.com/retain) and you have an effective churn management strategy. Using a billing platform that enables a [hybrid billing approach](https://www.paddle.com/blog/hybrid-billing-for-project-management-software), allowing you to flex between charging via card or invoice to accommodate your buyers' preferences, improves customer satisfaction while maintaining a singe source of revenue truth.

By mixing payment methods, you’ll need to set up your subscription billing and reporting tooling to [unify subscription payments](https://www.paddle.com/features/subscription-management/#71Gk4E5A4eyelA8vwvh2af) and avoid a “split-brain” finance team.
...
The most effective methods we've tested to increase [payment acceptance](https://www.paddle.com/blog/payment-acceptance):
...
Across our volumes and purchase types ([B2B/B2C](https://www.paddle.com/resources/b2b-b2c-churn), 1st checkout/monthly subscription/annual, AMEX/Visa/MasterCard), we typically see **more than 2X the proportion of failed payments for card transactions than PayPal**.
...
Estimate annual impact = 30.6% decrease in year-end ARR churn
 ...
 _Compound the monthly impact as 1 - (1-0.03)^12_
...
Alternatively, you could [migrate your subscriptions to a platform like Paddle,](https://www.paddle.com/solutions/reduce-churn) which manages all global acquiring and subscriptions together in one place — **no extra tools or business entities to integrate**. Paddle holds relationships with banks and card schemes worldwide and routes payments through entities on your behalf without any extra admin on your side.
...
Based on the benchmarks you’ve seen above (plus a few others) and your own data, wouldn’t it be helpful to get the best strategies to [reduce churn](https://www.paddle.com/blog/net-revenue-retention-the-new-benchmark-metric-for-saas) quantified for you?

Introducing the [Dollar Retention Calculator](https://paddlecom.typeform.com/to/NDPYQ3).

In five short questions, we’ll email you a personalized report based on your current monthly churn rate and calculate how much different [churn reduction](https://www.paddle.com/blog/paddle-acquires-profitwell) strategies will decrease your monthly churn and improve your [dollar retention rate](https://www.paddle.com/blog/net-dollar-retention).
...
You can calculate your gross dollar retention rate (excluding account expansion and upsells — that’s  _net dollar retention)_ by compounding your monthly churn rate over 12 months.
...
With the [Dollar Retention Calculator](https://paddlecom.typeform.com/to/NDPYQ3), you’ll get a list of which strategies to [reduce churn](https://www.paddle.com/blog/coming-soon-profitwell-metrics-integration) are more effective for you, and an estimated revenue gain from all of them together.
...
![](https://images.prismic.io/paddle/f05cd32c-b4f3-4325-9f7a-68458d1c54d8_churn+matrix.jpeg?auto=format%2Ccompress&fit=max&w=3840)
...
Try to [quantify which strategies might be most effective](https://paddlecom.typeform.com/to/NDPYQ3).
...
![](/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fbg-retain-banner.18741d29.jpg&w=3840&q=75)
